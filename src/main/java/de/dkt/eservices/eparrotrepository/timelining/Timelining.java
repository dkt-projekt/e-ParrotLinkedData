		...
		try{
			...
			for (TimelinedElement tle: listNifModels) {
				...
				if(meanDate!=null){
					...
					TimelinedElement te = new TimelinedElement(TimeElementType.DOCUMENT, NIFReader.extractDocumentURI(model), ter, model);
...
//				System.out.println(s);
				...
				if(teExist){
					TimelinedElement te = new TimelinedElement(TimeElementType.CONCEPT, uri, ter, model);
					listConcepts.add(te);
				}
			}
			return listConcepts;
